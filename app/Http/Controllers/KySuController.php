...
class KySuController extends Controller
{
    public function index()
    {
        $logo = DB::table('zm_logos')->take(1)->orderBy('id','desc')->get();
        $dclienhe = DB::table('zm_dclienhes')->where('stt','=','1')->orderBy('id','desc')->get();
        $tintuc = DB::table('zm_tintucs')->where([['stt','=','1'],['id_loaitintuc','=','4']])->orderBy('id','desc')->paginate(8);
        // $tintucorther = DB::table('zm_tintucs')->where([['stt','=','1'],['id_loaitintuc','=','4']])->skip(1)->take(4)->orderBy('id','desc')->get();
        ...
        $bannerpage = DB::table('zm_bannerpage')->where('page', '=', 'Kỹ sư')->first();
